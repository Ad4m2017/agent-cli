...
- `commandTimeoutMs` (`number`) -- Timeout fuer Tool-Command-Ausfuehrung (`run_command`) in Millisekunden. Effektiver Bereich: `100..10800000` (wird begrenzt). Fallback-Standard: `10000`.
...
- Die `--stats` Text-Ausgabe enthaelt Gesamtwerte, einen Quality-Bereich (`Retry Rate`, `Tool Failure Rate`) sowie `By Provider`/`By Model` und zeigt Rohwert + kompakte Form (z.B. `12345 (12.3k)`).
